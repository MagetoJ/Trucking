...
router.post('/login', async (req, res) => {
    try {
        ...
        const phoneNumber = (0, libphonenumber_js_1.parsePhoneNumberFromString)(phone, 'KE');
        const formattedPhone = phoneNumber ? phoneNumber.number : phone;
        const user = await db_1.db.user.findUnique({ where: { phone: formattedPhone } });
        if (!user)
...
router.post('/register', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: newUser.id, role: newUser.role, phone: newUser.phone }, JWT_SECRET, { expiresIn: '7d' });
        res.cookie('token', token, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            maxAge: 7 * 24 * 60 * 60 * 1000,
