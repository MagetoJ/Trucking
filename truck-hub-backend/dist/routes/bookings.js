 ...
 */
function isLoadOnSameRoute(activeOrigin, activeDest, newOrigin, newDest) {
    ...
    if (aoIdx === -1 || adIdx === -1 || noIdx === -1 || ndIdx === -1)
        return false;
    ...
    if (isActiveForward !== isNewForward)
        return false;
    if (isActiveForward) {
...
router.get('/', auth_1.requireAuth, async (req, res) => {
    ...
    const queryRole = req.query.role || req.user.role.toLowerCase();
    try {
        let bookings;
        if (queryRole === 'shipper') {
            bookings = await db_1.db.booking.findMany({
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const drivers = await db_1.db.user.findMany({ where: { role: 'DRIVER' } });
        for (const driver of drivers) {
            await createNotification(driver.id, 'New Load Broadcasted 📦', `A load request from ${origin} to ${destination} is now open for bidding.`);
        }
...
router.post('/:id/accept', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        const targetLoad = await db_1.db.booking.findUnique({ where: { id: bookingId } });
        if (!targetLoad)
            return res.status(404).json({ error: 'Load manifest not found.' });
        if (targetLoad.status !== 'PENDING')
            return res.status(410).json({ error: 'This load has already been claimed.' });
        // Look for any active, incomplete jobs currently handled by this driver
        const myActiveJobs = await db_1.db.booking.findMany({
            where: {
                driverId: userId,
        ...
        if (myActiveJobs.length > 0) {
            ...
            const routeCompatible = isLoadOnSameRoute(primaryJob.origin, primaryJob.destination, targetLoad.origin, targetLoad.destination);
            if (!routeCompatible) {
        ...
        const driverProfile = await db_1.db.user.findUnique({ where: { id: userId } });
        const vehicle = await db_1.db.vehicle.findFirst({ where: { driverId: userId } });
        ...
        await createNotification(targetLoad.shipperId, 'Driver Assigned 🚛', `Your shipment load has been claimed by driver: ${driverProfile?.name}. Contact details are now unlocked.`);
...
router.post('/:id/decline', auth_1.requireAuth, async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        const booking = await db_1.db.booking.findUnique({ where: { id: bookingId } });
        if (!booking)
        ...
        const rePooledBooking = await db_1.db.booking.update({
            where: { id: bookingId },
            data: {
                driverId: null,
                vehicleId: null,
        ...
        await createNotification(booking.shipperId, 'Load Re-pooled ⚠️', `The assigned driver has dropped the contract. Your load request has been returned to the public marketplace pool.`);
...
router.patch('/:id/confirm-delivery', auth_1.requireAuth, async (req, res) => {
    try {
        const booking = await db_1.db.booking.findUnique({ where: { id: parseInt(req.params.id, 10) } });
        if (!booking || booking.shipperId !== req.user.id)
            return res.status(403).json({ error: 'Unauthorized.' });
        const updated = await db_1.db.booking.update({
            where: { id: booking.id },
...
router.patch('/:id/arrive-destination', auth_1.requireAuth, async (req, res) => {
    try {
        const booking = await db_1.db.booking.findUnique({ where: { id: parseInt(req.params.id, 10) } });
        if (!booking || booking.driverId !== req.user.id)
            return res.status(403).json({ error: 'Unauthorized.' });
        const updated = await db_1.db.booking.update({
            where: { id: booking.id },
