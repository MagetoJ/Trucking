...
exports.default = DriverKycOnboardingHub;
const react_1 = require("react");
const button_1 = require("@/components/ui/button");
const fetcher_1 = require("@/lib/fetcher");
const store_1 = require("@/lib/store"); // Import AuthState
const lucide_react_1 = require("lucide-react");
function DriverKycOnboardingHub() {
    // Explicitly type the state parameter for better type safety
    const token = (0, store_1.useAuthStore)((state) => state.token);
    const user = (0, store_1.useAuthStore)((state) => state.user);
    const [step, setStep] = (0, react_1.useState)(1);
    const [isProcessing, setIsProcessing] = (0, react_1.useState)(false);
    const [telemetryLogs, setTelemetryLogs] = (0, react_1.useState)([]);
    // Local Form Buffer Mappings
    const [enteredOtp, setEnteredOtp] = (0, react_1.useState)('');
    const [nationalIdNum, setNationalIdNum] = (0, react_1.useState)('');
    const [carrierLicenseRef, setCarrierLicenseRef] = (0, react_1.useState)('');
    const [licensePlateString, setLicensePlateString] = (0, react_1.useState)('');
    const logEvent = (message) => setTelemetryLogs(prev => [`[${new Date().toLocaleTimeString()}] ${message}`, ...prev]);
    const requestEmailVerificationCode = async () => {
        setIsProcessing(true);
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/step1-send-email-otp`, {
                method: 'POST',
            ...
            logEvent('SMTP network pipeline connection error.');
        }
        finally {
    ...
    const verifyEmailOtpHandshake = async () => {
        setIsProcessing(true);
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/step2-verify-email-token`, {
                method: 'POST',
            ...
            if (res.ok) {
                ...
                logEvent('Verification failed: Invalid token string.');
            }
        }
        catch {
            logEvent('Handshake execution exception.');
        }
        finally {
            setIsProcessing(false);
        }
    ...
    const runIntelligentOcrCheck = async () => {
        setIsProcessing(true);
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/step3-process-ocr`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            ...
            logEvent('OCR module response timeout exception.');
        }
        finally {
            setIsProcessing(false);
        }
    ...
    const saveFleetCredentials = async () => {
        setIsProcessing(true);
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/step4-fleet-credentials`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            ...
            logEvent('Credential database post error.');
        }
        finally {
            setIsProcessing(false);
        }
    ...
    const processFacialCrossMatch = async () => {
        setIsProcessing(true);
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/step5-facial-biometrics`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            ...
            logEvent('Biometric pipeline lookup crashed.');
        }
        finally {
            setIsProcessing(false);
        }
    ...
    const executePayoutTestPush = async () => {
        setIsProcessing(true);
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/step6-wallet-test-deposit`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await res.json();
            ...
            logEvent('M-Pesa system timeout exception error.');
        }
        finally {
            setIsProcessing(false);
        }
    };
    const triggerIsolatedNodeReset = async (nodeKey) => {
        try {
            const res = await fetch(`${fetcher_1.BACKEND_BASE_URL}/api/kyc/reset-isolated-document-node`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                body: JSON.stringify({ documentTargetKey: nodeKey })
            });
            if (res.ok)
                logEvent(`Resiliency Override Engine: Cleared single database matrix state cell ['${nodeKey}'] back to PENDING. Flow sequence remains un-purged.`);
        }
        catch {
            logEvent('Override exception.');
        }
    };
    return (<div className="grid lg:grid-cols-5 gap-6 max-w-6xl mx-auto items-start pb-12">
        ...
        <div className="w-[315px] h-[630px] bg-slate-950 border-8 border-slate-900 rounded-[38px] shadow-2xl overflow-hidden flex flex-col relative">
          ...
          <div className="flex-1 p-5 space-y-4 flex flex-col justify-between bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950">
            <div>
              ...
              {step === 1 && (<div className="space-y-4 text-center pt-6 animate-fade-in">
                  <lucide_react_1.Mail className="w-12 h-12 text-orange-500 mx-auto"/>
                  ...
                  <button_1.Button onClick={requestEmailVerificationCode} disabled={isProcessing} className="w-full bg-orange-500 hover:bg-orange-600 text-slate-950 text-xs font-black py-2.5 rounded-xl cursor-pointer">
                    {isProcessing ? 'Transmitting Token...' : 'Request Email Code'}
                  </button_1.Button>
                </div>)}

              {step === 2 && (<div className="space-y-3 pt-2 animate-fade-in">
                  ...
                  <button_1.Button onClick={verifyEmailOtpHandshake} disabled={isProcessing || enteredOtp.length !== 6 || !nationalIdNum} className="w-full bg-orange-500 text-slate-950 text-xs font-black py-2.5 rounded-xl mt-1 cursor-pointer">
                    Verify Registration Assets
                  </button_1.Button>
                </div>)}

              {step === 3 && (<div className="space-y-4 text-center pt-4 animate-fade-in">
                  <lucide_react_1.FileText className="w-12 h-12 text-indigo-400 mx-auto"/>
                  ...
                  <button_1.Button onClick={runIntelligentOcrCheck} disabled={isProcessing} className="w-full bg-orange-500 text-slate-950 text-xs font-black py-2.5 rounded-xl cursor-pointer">
                    Execute Automated OCR Matrix
                  </button_1.Button>
                </div>)}

              {step === 4 && (<div className="space-y-3 pt-2 animate-fade-in">
                  ...
                  <button_1.Button onClick={saveFleetCredentials} disabled={isProcessing || !carrierLicenseRef || !licensePlateString} className="w-full bg-orange-500 text-slate-950 text-xs font-black py-2.5 rounded-xl mt-1 cursor-pointer">
                    Save Specifications
                  </button_1.Button>
                </div>)}

              {step === 5 && (<div className="space-y-4 text-center pt-6 animate-fade-in">
                  <lucide_react_1.Camera className="w-12 h-12 text-teal-400 mx-auto"/>
                  ...
                  <button_1.Button onClick={processFacialCrossMatch} disabled={isProcessing} className="w-full bg-orange-500 text-slate-950 text-xs font-black py-2.5 rounded-xl cursor-pointer">
                    Perform Identity Comparison Check
                  </button_1.Button>
                </div>)}

              {step === 6 && (<div className="space-y-4 text-center pt-6 animate-fade-in">
                  <lucide_react_1.ShieldCheck className="w-12 h-12 text-emerald-400 mx-auto"/>
                  ...
                  <button_1.Button onClick={executePayoutTestPush} disabled={isProcessing} className="w-full bg-orange-500 text-slate-950 text-xs font-black py-2.5 rounded-xl cursor-pointer">
                    Authorize Billing Verification Check
                  </button_1.Button>
                </div>)}

              {step === 7 && (<div className="space-y-4 text-center pt-10 animate-fade-in">
                  <lucide_react_1.CheckCircle className="w-12 h-12 text-orange-500 mx-auto"/>
      ...
      <div className="lg:col-span-3 flex flex-col justify-between bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl min-h-[630px]">
        <div className="space-y-5">
          ...
          <div className="p-4 bg-slate-950 border border-slate-850 rounded-xl space-y-2">
            <h4 className="text-xs font-bold uppercase tracking-wide text-orange-400 flex items-center gap-1.5 font-mono">
              <lucide_react_1.AlertCircle className="w-4 h-4"/> Multi-Resiliency Recovery Matrix
            </h4>
        ...
        ].map((node) => (<button key={node.key} onClick={() => triggerIsolatedNodeReset(node.key)} className="px-2.5 py-1.5 bg-slate-900 border border-slate-800 rounded-lg font-mono text-[10px] text-slate-200 hover:border-orange-500 transition cursor-pointer">
                  {node.label}
                </button>))}
            </div>
          </div>
    ...
    // Intercepting system telemetry actions pipeline hooks...</p>
          {telemetryLogs.map((log, index) => (<p key={index} className="text-indigo-400 leading-tight">➔ {log}</p>))}
        </div>
      </div>
